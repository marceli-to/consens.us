<div>
    <section class="pb-12">
        ...
        <div class="space-y-4 mb-12">
            <div>
                <label class="block text-[10px] font-medium uppercase tracking-[0.2em] text-ink-muted mb-2">Öffentlicher Link</label>
                <div class="flex gap-2">
                    <input type="text" value="{{ $publicUrl }}" readonly
                        class="flex-1 px-4 py-3 text-sm border border-rule bg-cream-dark text-ink-muted focus:outline-none">
            ...
            <div>
                <label class="block text-[10px] font-medium uppercase tracking-[0.2em] text-ink-muted mb-2">Admin Link <span class="text-ink-faint">(nur für dich)</span></label>
                <div class="flex gap-2">
        ...
        <div class="space-y-6 mb-12 pb-12 border-b border-rule">
            <div>
                <label class="block text-[10px] font-medium uppercase tracking-[0.2em] text-ink-muted mb-2">Titel</label>
                <input type="text" wire:model="title"
                    class="w-full px-4 py-3 text-sm border border-rule bg-cream text-ink focus:outline-none focus:border-ink transition-colors">
            </div>
            <div>
                <label class="block text-[10px] font-medium uppercase tracking-[0.2em] text-ink-muted mb-2">Beschreibung</label>
                <textarea wire:model="description" rows="3"
                    class="w-full px-4 py-3 text-sm border border-rule bg-cream text-ink focus:outline-none focus:border-ink transition-colors resize-none"></textarea>
            </div>
                ...
                @foreach($options as $option)
                    <div class="flex items-center justify-between py-2 px-4 border border-rule">
                        ...
                        <button wire:click="removeOption({{ $option['id'] }})"
                            class="text-[10px] uppercase tracking-[0.2em] text-ink-faint hover:text-accent transition-colors cursor-pointer">
                            Entfernen
                        </button>
                    </div>
                @endforeach
